...
def get_veikkaus_odds():
	...
	for game in games:
		...
		home_win = float(BeautifulSoup(odds[0].get_attribute("innerHTML"), 'html.parser').find("span").text.replace(',', '.'))
		tie = float(BeautifulSoup(odds[1].get_attribute("innerHTML"), 'html.parser').find("span").text.replace(',', '.'))
		away_win = float(BeautifulSoup(odds[2].get_attribute("innerHTML"), 'html.parser').find("span").text.replace(',', '.'))

		ret_list.append([home_team, away_team, home_win, tie, away_win])
